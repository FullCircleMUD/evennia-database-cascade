...
"""Minimal Django settings for evennia-database-cascade unit tests.
...
The library's own subject matter is the DATABASES dict, so note what this file
deliberately is not: the suite builds the environments it resolves against and
passes them in as arguments. Nothing here stands in for a deployment.
"""
import os
import sys
import tempfile
...
LOG_DIR = os.path.join(tempfile.gettempdir(), "evennia_database_cascade_test_logs")
os.makedirs(LOG_DIR, exist_ok=True)

# Library under test
INSTALLED_APPS = list(INSTALLED_APPS) + ["evennia_database_cascade"]
